...
static results_t timers_stats(std::array<double, NB_ITER> const &timers)
{
    ...
    auto const sum = std::reduce(timers.begin(), timers.end(), 0.0, std::plus<>{});
    auto const mean = sum / double(timers.size());
    auto const sdev =
        sqrt(std::accumulate(timers.begin(), timers.end(), 0.0,
...
int main(int argc, char *argv[])
{
    ...
    for (size_t t = 0; t < NB_ITER; ++t) {
        ...
        else if (MODE == "8stores_profiling") {
